...
public class Lexer {
    ...
    // positions in line of current token, and current line number
    ...
    public static void main(String args[]) {
        ...
        if (args.length!=1){
            System.out.println("Not the proper usage dude, try: java Lexer <filename>");
            System.exit(-1);
        } 
        ...
        try {
            ...
            while (true) {
                ...
                if ((tok.getKind() == Tokens.Identifier) ||
                    ...
                    System.out.printf("%-10s" ,tok.toString());
                } else {
                    System.out.printf("%-10s" ,tok.getKind().toString());
                }
...
*/
    public Token newIntegerToken(String number,int startPosition,int endPosition) {
        return new Token(startPosition,endPosition,source.getLineno(),
            Symbol.symbol(number,Tokens.INTEGER));
    }
    
 /**
  * float tokens are insterted into the symbol table, but are not converted to
  * the actual floating point value. 
  *  @param floatNum is the float String just scanned, i.e. "3.8"
  ...
  */   
    public Token newFloatToken(String floatNum,int startPosition,int endPosition) {
        return new Token(startPosition,endPosition, source.getLineno(),
            Symbol.symbol(floatNum,Tokens.FLOAT));
    }
...
*/
    public Token nextToken() { // ch is always the next char to process
        if (atEOF) {
            if (source != null) {
                source.printSource(); // print out the whole program that was read in before quiting. 
                source.close();
                source = null;
            }
            return null;
        ...
        // can return either an integer token or a float.
        ...
        // will assume that it what follows is a float, if not then it will
        // pass the token to makeToken to print out error. 
        if (ch =='.'){
            String FLoat = "";
            int length = 0;
            try {
            ...
            // if there are no digits after the '.', then we send it to makeToken
            // to throw the error. 
            if(length==1) return makeToken(FLoat,startPosition,endPosition);
            return newFloatToken(FLoat,startPosition,endPosition);
        }
